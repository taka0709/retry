...
FLUIDBOX;
	...
	if( isset( $luxe['sns_bottoms_enable'] ) && isset( $luxe['sns_bottoms_count'] ) ) {
		if( $luxe['sns_bottoms_type'] === 'color' || $luxe['sns_bottoms_type'] === 'white' ) {
			$color = $luxe['sns_bottoms_type'] === 'color' ? '.sns-c' : '.sns-w';
	...
	if( isset( $luxe['sns_tops_enable'] ) ) {
		if( $luxe['sns_tops_type'] === 'flatc' || $luxe['sns_tops_type'] === 'flatw' ) {
			$style = sns_adjust_width( $style, 'tops', 'flat' );
		}
		elseif( $luxe['sns_tops_type'] === 'iconc' || $luxe['sns_tops_type'] === 'iconc' ) {
			$style = sns_adjust_width( $style, 'tops', 'icon' );
		}
	}
	// SNS 下ボタン幅の調整
	if( isset( $luxe['sns_bottoms_enable'] ) ) {
		if( $luxe['sns_bottoms_type'] === 'flatc' || $luxe['sns_bottoms_type'] === 'flatw' ) {
			$style = sns_adjust_width( $style, 'bottoms', 'flat' );
		}
		elseif( $luxe['sns_bottoms_type'] === 'iconc' || $luxe['sns_bottoms_type'] === 'iconc' ) {
			$style = sns_adjust_width( $style, 'bottoms', 'icon' );
		}
	}
...
function sns_adjust_width( $style, $position, $type ) {
	global $luxe;
	...
	if( $type === 'icon' ) {
		if( $buttons >= 9 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 20%}';
			$style['max_575'] .= '#sns-' . $position . ' li{min-width: 20%}';
		}
		elseif( $buttons >= 8 ) {
			$style['max_575'] .= '#sns-' . $position . ' li{min-width: 25%}';
		}
	}
	else {
		if( $buttons >= 9 ) {
		...
		elseif( $buttons >= 8 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 25%}';
			$style['max_575'] .= '#sns-' . $position . ' li{min-width: 25%}';
		}
		elseif( $buttons >= 7 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 14.2%}';
			$style['max_575'] .= '#sns-' . $position . ' li{min-width: 25%}';
		}
		elseif( $buttons >= 6 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 16.6%}';
			$style['max_575'] .= '#sns-' . $position . ' li{min-width: 33.3%}';
		}
		elseif( $buttons >= 5 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 20%}';
		}
		...
		elseif( $buttons >= 3 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 33.3%}';
		}
		elseif( $buttons >= 2 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 50%}';
		}
		elseif( $buttons >= 1 ) {
			$style['all'] .= '#sns-' . $position . ' li{min-width: 100%}';
		}
	}
