...
class description_walker extends Walker_Nav_Menu {
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( !empty( $item->attr_title ) ) {
			...
			$attributes .= ' title="' . apply_filters( 'the_title', $item->title ) . '"';
		}

		$item_output = $args->before
		.	'<a' . $attributes . '>'
		.	$args->link_before . apply_filters( 'the_title', $item->title ) . $args->link_after
		.	'</a>'
		.	$args->after;
...
// User Custom Menu
$wp_nav_menu = wp_nav_menu( array ( 'theme_location' => 'head-band', 'echo' => false, 'container' => false, 'fallback_cb' => false, 'items_wrap' => '<div><ul>%3$s', 'walker' => new Description_Walker() ) );
